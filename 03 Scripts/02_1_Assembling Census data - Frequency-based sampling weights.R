#########################################################
#  Part 2.1: Assembling Census data - Frequency -baed sampling weights
...
viviendas_vacantes_rm<- censo_tabla("zonas") %>%
  mutate(
  ...
  inner_join(select(censo_tabla("viviendas"),zonaloc_ref_id,vivienda_ref_id,p01,p02),by='zonaloc_ref_id')%>%
  collect()
