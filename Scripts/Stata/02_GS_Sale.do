...
global pathRes "${path}04 Results/02_GS/Sale/"
	...
	while `y'<2021{
		...
		while `m'<13{
			...
			* Caliper matching on the propensity scores (k nearest neighboors pscore + cal)
			...
			drop if _support ==0
			drop _treated _support _weight _n1 _nn _precio _id _pdif
			...
			*Fisher index over One2One
			regress lnCP dormitorios banos sqft longitud latitud idtipopropiedad [fweight = _weight] if treated==1
				est store One2OneHedReg
				est table 
				est store betR
				matrix bet=r(coef)
				tabstat dormitorios banos  sqft longitud latitud idtipopropiedad [fweight = _weight], statistics(mean) columns(variables) save, if treated==1
				matrix Mean_r=r(StatTotal)
				matrix Mean_T= Mean_r, 1
					tabstat dormitorios banos sqft longitud latitud idtipopropiedad [fweight = _weight], statistics(var) columns(variables) save, if treated==1
			...
			regress lnCP dormitorios banos sqft longitud latitud idtipopropiedad [fweight = _weight] if treated==0
				est store One2OneHedRegNT
				est table 
				est store betR_NT
				matrix bet_NT=r(coef)
				tabstat dormitorios banos sqft longitud latitud idtipopropiedad [fweight = _weight], statistics(mean) columns(variables) save, if treated==0
				matrix Mean_NT=r(StatTotal)
				matrix Mean_NT2= Mean_NT, 1
					tabstat dormitorios banos sqft longitud latitud idtipopropiedad [fweight = _weight], statistics(var) columns(variables) save, if treated==0
			...
			* Fisher Index2
			...
			quietly sum `a' [fweight = _weight] if treated==1
			scalar meanT_`a'=r(mean)
			quietly sum `a' [fweight = _weight] if treated==0
			scalar meanC_`a'=r(mean)
			}
			scalar nlp_r= nlp_r[1,1]
			scalar nlp_NT2= nlp_NT2[1,1]
			xi: regress lnCP i.treated|dormitorios i.treated|banos i.treated|sqft i.treated|longitud i.treated|latitud  i.treated|idtipopropiedad treated [fweight = _weight] 		
			...
			* Standarized differences between T and NT variables used for the matching
			...
			mat coln StandD = bed bath IntFootage
			mat rown StandD = "`y' `m'"
