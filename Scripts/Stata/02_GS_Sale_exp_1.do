...
global pathData "${path}01 Data/02_GS/"
global pathRes "${path}04 Results/02_GS/Sale_exp_1/"
...
* no sufficient obserations on Vitacura, Colina, Vina, San Miguel, Penalolen, La Reina, Puente Alto, Nunoa, Macul, Valparaiso, Antofagasta, La Florida on 2018/04
	...
	while `y'<2021{
		...
		while `m'<13{
			...
			*local y=2020
			*local m=1
			insheet using "${pathData}GS_`y'_`m'.csv", comma clear
			noi di in red "1_`y'_`m'.csv"
			...
			* getting number of control and treated before matching
			...
			mat rown NumbB_T = "1_`y'_`m'"
			mat rown NumbB_NT = "1_`y'_`m'"
			...
			* Median Price index
			...
			mat2txt , m(Med_Pri) sav("${pathRes}1_Med_Ind.txt") append
			...
			* Caliper matching on the propensity scores (k nearest neighboors pscore + cal)
			...
			order expansor _id _n1, last
			
			* replacing expansor from the matched obs
			drop if 	expansor==. & _n1==.
			gen 	id2=cond(missing(expansor), _n1, _id)
			bys 	id2 (expansor): replace expansor=expansor[1] if missing(expansor)
			sort 	_id
			...
			*getting number of obs by treat/control after matching
			...
			mat rown NumbA_T = "1_`y'_`m'"
			mat rown NumbA_NT = "1_`y'_`m'"
			mat2txt , m(NumbA_T) sav("${pathRes}NumbA_T.txt") append
			mat2txt , m(Count) sav("${pathRes}1_CountB&A.txt") append
			...
			*drop if _support ==0
				...
				mat2txt , m(TreatStat) sav("${pathRes}1_TreatStatMSA.txt") append
				tabstat ps, stats(mean)save by(_treated)
				mat NT=r(Stat1)
				mat TT=r(Stat2)	
				mat PSDiff=TT-NT
				mat rown PSDiff = "`y' `m'"
				mat2txt , m(PSDiff) sav("${pathRes}1_PSDiffMSA.txt") append
				gsort -_treated
				gen lnCP = ln(preciouf)
			*Fisher index over One2One
			regress lnCP dormitorios banos sqft longitud latitud idtipopropiedad [pweight = expansor] if treated==1
				est store One2OneHedReg
				est table 
				est store betR
				matrix bet=r(coef)
				tabstat dormitorios banos  sqft longitud latitud idtipopropiedad [fweight = round(expansor)], statistics(mean) columns(variables) save, if treated==1
				matrix Mean_r=r(StatTotal)
				matrix Mean_T= Mean_r, 1
					tabstat dormitorios banos sqft longitud latitud idtipopropiedad [fweight = round(expansor)], statistics(var) columns(variables) save, if treated==1
			...
			regress lnCP dormitorios banos sqft longitud latitud idtipopropiedad [pweight = expansor] if treated==0
				est store One2OneHedRegNT
				est table 
				est store betR_NT
				matrix bet_NT=r(coef)
				tabstat dormitorios banos sqft longitud latitud idtipopropiedad [fweight = round(expansor)], statistics(mean) columns(variables) save, if treated==0
				matrix Mean_NT=r(StatTotal)
				matrix Mean_NT2= Mean_NT, 1
					tabstat dormitorios banos sqft longitud latitud idtipopropiedad [fweight = round(expansor)], statistics(var) columns(variables) save, if treated==0
			...
			* Fisher index based on hedonic regressions over matched databases
			...
			mat coln Hed = "HPI"
			mat rown Hed = "`y' `m'"
			mat2txt , m(Hed) sav("${pathRes}1_HedMSA.txt") append
			* Fisher Index2
			foreach a in dormitorios banos sqft longitud latitud idtipopropiedad{
			quietly sum `a' [fweight = round(expansor)] if treated==1
			scalar meanT_`a'=r(mean)
			quietly sum `a' [fweight = round(expansor)] if treated==0
			scalar meanC_`a'=r(mean)
			}
			scalar nlp_r= nlp_r[1,1]
			scalar nlp_NT2= nlp_NT2[1,1]
			xi: regress lnCP i.treated|dormitorios i.treated|banos i.treated|sqft i.treated|longitud i.treated|latitud  i.treated|idtipopropiedad treated [pweight = expansor] 		
			...
			mat2txt , m(Hed2_Sd2) sav("${pathRes}1_HedPrice2_Sd2.txt") append
			* Standarized differences between T and NT variables used for the matching
			...
			mat rown StandD = "`y' `m'"
			mat2txt , m(StandD) sav("${pathRes}1_StandDMSA.txt") append
			local m=`m'+1
		}
		local y=`y'+1
	}
